define(function(require, exports, module) {
	...
	function setInternal(self, name, value, opts, attrs) {
		...
		var beforeEventObject = Tools.mix({
			attrName: name,
	...
	Attribute.extend = function extend(px, sx) {
		...
		if (px.hasOwnProperty('constructor')) {
			...
			if ('@DEBUG@') {
				/*jshint evil: true*/
				SubClass = new Function('return function ' + Tools.camelCase(name) + '(){ ' + 'this.callSuper.apply(this, arguments);' + '}')();
			} else {
				SubClass = function() {
		...
		if (inheritedStatics = self.inheritedStatics) {
			...
			Tools.mix(sxInheritedStatics, inheritedStatics, false);
		}
		Tools.extend(SubClass, self, px, sx);
		if (sxInheritedStatics) {
			Tools.mix(SubClass, sxInheritedStatics);
		}
	...
	Tools.augment(Attribute, CustomEvent.Target, {
		INVALID: INVALID,
		callSuper: function() {
		...
		addAttrs: function(attrConfigs, initialValues) {
			var self = this;
			Tools.each(attrConfigs, function(attrConfig, name) {
		...
		set: function(name, value, opts) {
			...
			if (typeof name !== 'string') {
				...
				var attrNames = [],
					...
					newVals = [],
					subAttrNames = [];
				Tools.each(attrs, function(attr) {
