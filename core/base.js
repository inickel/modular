define(function(require, exports, module) {
	...
	var Base = Attribute.extend({
		...
		unplug: function(plugin) {
			...
			Tools.each(self.get('plugins'), function(p) {
				var keep = 0,
					pluginId;
				if (plugin) {
					if (isString) {
		...
		getPlugin: function(id) {
			var plugin = null;
			Tools.each(this.get('plugins'), function(p) {
				// user defined takes priority
		...
		destroy: function() {
			var self = this,
				args = Tools.makeArray(arguments);
	...
	Tools.mix(Base, {
		...
		extend: function extend(extensions, px, sx) {
			...
			if (extensions.length) {
				...
				// 主类最优先
				Tools.each(extensions.concat(SubClass), function(ext) {
					if (ext) {
						// 合并 ATTRS 到主类
						...
						Tools.each(ext.ATTRS, function(v, name) {
				...
				Tools.augment(SubClass, prototype);
			}
	...
	function constructPlugins(self) {
		var plugins = self.get('plugins'),
			Plugin;
		Tools.each(plugins, function(plugin, i) {
			if (typeof plugin === 'function') {
				Plugin = plugin;
